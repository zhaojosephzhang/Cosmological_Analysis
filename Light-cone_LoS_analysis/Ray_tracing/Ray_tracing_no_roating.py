"""Build a no-rotation FRB DM light cone from CROCODILE fullbox snapshots.

This version keeps the periodic box orientation fixed and optionally restricts
the sampled sightlines to the CHIME declination window.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description='Build a no-rotation DM light cone from CROCODILE grid data.')
    parser.add_argument('--base-dir', required=True, help='Directory containing fullbox_###_*_lv*.hdf5 snapshots')
    parser.add_argument('--agn-tag', default='n', help='Feedback tag in snapshot filenames, e.g. n or f')
    parser.add_argument('--level', default='7', help='Grid refinement level used in the filenames')
    ...
    parser.add_argument('--nlos', type=int, default=50000, help='Number of lines of sight to generate')
    parser.add_argument('--z-max', type=float, default=1.0, help='Maximum redshift of the cone')
    parser.add_argument('--step-in-cells', type=float, default=1.0, help='Internal radial step in units of the grid cell size')
    parser.add_argument('--min-bins', type=int, default=300, help='Minimum number of internal radial bins')
    parser.add_argument('--overlap-bins', type=int, default=16, help='Number of blending bins across snapshot boundaries')
    parser.add_argument('--n-z-out', type=int, default=300, help='Number of DM(z) samples saved to the output file')
    parser.add_argument('--seed', type=int, default=0, help='Random seed for observer and sky sampling')
    parser.add_argument('--use-chime-window', action='store_true', help='Restrict directions to the CHIME declination window')
    parser.add_argument('--dec-min', type=float, default=-9.5, help='Minimum declination in degrees for CHIME selection')
    parser.add_argument('--dec-max', type=float, default=90.0, help='Maximum declination in degrees for CHIME selection')
...
def chime_window_directions(n_dir: int, dec_min_deg: float, dec_max_deg: float, rng: np.random.Generator) -> np.ndarray:
    smin = np.sin(np.deg2rad(dec_min_deg))
    smax = np.sin(np.deg2rad(dec_max_deg))
    u = rng.uniform(smin, smax, size=n_dir)
    dec = np.arcsin(u)
    ra = rng.uniform(0.0, 2.0 * np.pi, size=n_dir)
    n_hat = np.zeros((n_dir, 3), dtype=np.float64)
    n_hat[:, 0] = np.cos(dec) * np.cos(ra)
    n_hat[:, 1] = np.cos(dec) * np.sin(ra)
    n_hat[:, 2] = np.sin(dec)
    return n_hat
...
class RayConeConfig:
    ...
    use_chime_window: bool = False
    dec_min_deg: float = -9.5
...
def build_dm_cone(cfg: RayConeConfig) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, Dict]:
    ...
    if cfg.use_chime_window:
        n_hat = chime_window_directions(cfg.n_dir, cfg.dec_min_deg, cfg.dec_max_deg, rng)
    else:
        n_hat = fibonacci_sphere_directions(cfg.n_dir, rng)
    cell_size_code = float(grids[0].cell_size_code)
    dchi_step_mpc = cfg.step_in_cells * (cell_size_code / h)
    chi_max = comoving_distance(cfg.z_max, omega_m, omega_l, h0)
    nbin = max(cfg.min_bins, int(np.ceil(chi_max / dchi_step_mpc)))
    ...
    out_idx = np.unique(np.linspace(0, nbin - 1, max(1, cfg.n_out), dtype=np.int64))
    ...
    meta = {
        ...
        'mapping': 'periodic_wrap_only',
        'dm_units': 'pc cm^-3',
        'use_chime_window': bool(cfg.use_chime_window),
        'dec_min_deg': float(cfg.dec_min_deg),
        'dec_max_deg': float(cfg.dec_max_deg),
    }
...
def save_dm_cone_hdf5(out_path: str, cfg: RayConeConfig, dm_tot: np.ndarray, dm_cum: np.ndarray, z_prof: np.ndarray, n_hat: np.ndarray, meta: Dict) -> str:
    ...
    with h5py.File(out_path, 'w') as handle:
        ...
        g_header.attrs['UseCHIMEWindow'] = bool(cfg.use_chime_window)
        g_header.attrs['CHIME_dec_min_deg'] = float(cfg.dec_min_deg)
        g_header.attrs['CHIME_dec_max_deg'] = float(cfg.dec_max_deg)
...
def default_output_path(cfg: RayConeConfig) -> Path:
    ...
    return Path(cfg.base_dir) / 'Ray_tracing' / f'dm_cone_no_rotation_zmax{z_str}_lv{cfg.level_input}_{cfg.agn_tag}_Nlos{cfg.n_dir}.hdf5'
...
def main() -> None:
    ...
    cfg = RayConeConfig(
        ...
        seed=args.seed,
        z_max=args.z_max,
        step_in_cells=args.step_in_cells,
        min_bins=args.min_bins,
        overlap_bins=args.overlap_bins,
        n_out=args.n_z_out,
        use_chime_window=args.use_chime_window,
